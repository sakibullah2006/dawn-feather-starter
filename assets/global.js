...
class StickyHeader {
  constructor() {
    this.header = document.querySelector(".shopify-section-header-sticky");
    if (!this.header) return;

    this.stickyType = this.header.dataset.stickyType;
    this.headerBounds = {};
  ...
  init() {
    // Make the header group sticky
    const headerGroup = this.header.closest(
      ".shopify-section-group-header-group",
    );
    if (headerGroup) {
      headerGroup.style.position = "sticky";
      headerGroup.style.top = "0";
      headerGroup.style.zIndex = "30";
    }

    if (this.stickyType === "always") {
      this.header.classList.add("shopify-section-header-sticky--stuck");
    }
  ...
  onScroll() {
    ...
    if (scrollTop > this.currentScrollTop && scrollTop > 150) {
      // Scrolling down
      if (this.preventReveal) {
        window.removeEventListener("scroll", this.hideHeaderOnScrollUp);
        this.preventReveal = false;
      }
      this.header.style.transform = "translateY(-100%)";
      this.header.classList.remove("shopify-section-header-sticky--stuck");
